...
public:
    NeuralNetwork() {
        layer1_output.resize(20);
        layer2_output.resize(4);
    }
    ...
    // Layer 1
    if (layer1_weights.size() != 20 || layer1_weights[0].size() != 8 || layer1_bias.size() != 20) {
    ...
    // Layer 2
    if (layer2_weights.size() != 4 || layer2_weights[0].size() != 20 || layer2_bias.size() != 4) {
